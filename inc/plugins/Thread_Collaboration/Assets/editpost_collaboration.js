    ...
    $(document).ready(function(){
        ...
        function initializeUsernameSelect2($element) {
            if ($.fn.select2) {
                ...
                $element.select2({
                    ...
                    multiple: false,
                    allowClear: true,
                    ajax: { url: 'xmlhttp.php?action=get_users', dataType: 'json', data: function(term){ return { query: term }; }, results: function(data){ return { results: data }; } },
                    initSelection: function(element, callback){ var value=$(element).val(); if(value!==''){ callback({id:value,text:value}); } },
                    createSearchChoice: function(term, data){ if($(data).filter(function(){ return this.text.localeCompare(term)===0; }).length===0){ return {id:term, text:term}; } }
                });
                $element.on('change', function(){ var value=$element.select2('val'); $element.val(value); });
            }
        }
        ...
        function initializeRoleSelect2($element) {
            if ($.fn.select2) {
                ...
                $element.on('select2-selecting', function(e){ var sel=e.choice; var $field=$(this).closest('.thread_collaboration_field'); var $label=$field.find('.icon-field-label'); var $icon=$field.find('.collaborator_role_icon'); var m=defaultRoles.find(function(r){return r.name===sel.text;}); if(m&&m.icon){ $icon.val(m.icon); $label.hide(); } else { $label.show(); $icon.val(''); } });
                $element.on('change', function(){ var v=$(this).val(); var $field=$(this).closest('.thread_collaboration_field'); var $label=$field.find('.icon-field-label'); var $icon=$field.find('.collaborator_role_icon'); var m=defaultRoles.find(function(r){return r.name===v;}); if(m&&m.icon){ $icon.val(m.icon); $label.hide(); } else if($.trim(v)!==''){ $label.show(); $icon.val(''); } else { $label.hide(); $icon.val(''); } });
            }
        }
        ...
        function initializeAdditionalRoleSelect2($element) {
            if ($.fn.select2) {
                ...
                $element.on('select2-selecting', function(e){ var sel=e.choice; var $row=$(this).closest('.additional-role-row'); var $label=$row.find('.additional-icon-field-label'); var $icon=$row.find('.collaborator_additional_role_icon'); var m=defaultRoles.find(function(r){return r.name===sel.text;}); if(m&&m.icon){ $icon.val(m.icon); $label.hide(); } else { $label.show(); $icon.val(''); } });
                $element.on('change', function(){ var v=$(this).val(); var $row=$(this).closest('.additional-role-row'); var $label=$row.find('.additional-icon-field-label'); var $icon=$row.find('.collaborator_additional_role_icon'); var m=defaultRoles.find(function(r){return r.name===v;}); if(m&&m.icon){ $icon.val(m.icon); $label.hide(); } else if($.trim(v)!==''){ $label.show(); $icon.val(''); } else { $label.hide(); $icon.val(''); } });
            }
        }
        ...
        $(document).on('change', '.enable_multiple_roles', function(){
            ...
            if ($(this).is(':checked')) {
                ...
                $additionalRoles.find('.collaborator_additional_role').each(function(){ try{ $(this).select2('destroy'); }catch(e){} });
            }
        });
